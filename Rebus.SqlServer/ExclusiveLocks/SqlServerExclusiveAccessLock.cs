...
/// <summary>
/// SQL Server implementation of <see cref="IExclusiveAccessLock"/> and generic exclusive lock support
/// </summary>
public class SqlServerExclusiveAccessLock: IExclusiveAccessLock, IInitializable, IDisposable
{
    /// <summary>
    ...
    /// Constructor
    /// </summary>
    /// <param name="connectionProvider">A <see cref="SqlServer.IDbConnection"/> to obtain a database connection</param>
        ...
        SqlServerExclusiveAccessLockOptions options)
    {
        ...
        _log = rebusLoggerFactory.GetLogger<SqlServerExclusiveAccessLock>();
    ...
    private void InnerEnsureTableIsCreated(TableName table)
    {
        using (var connection = _connectionProvider.GetConnection())
        {
            ...
            else
            {
                ...
                command.CommandText = @$"
                    CREATE TABLE {table.QualifiedName} (
                        [lock_key] NVARCHAR({LockKeyColumnSize}) NOT NULL,
                        [expiration] DATETIME NOT NULL,
                        CONSTRAINT [PK_{table.Name}LockKey] PRIMARY KEY CLUSTERED ([lock_key] ASC)
                    );
                    ----
                    CREATE NONCLUSTERED INDEX idx_expiration ON {table.QualifiedName} ([expiration] ASC);";

...
public class TestSagaInstanceLocking : FixtureBase
{
    ...
    protected override void SetUp()
    {
        ...
        SqlTestHelper.DropTable(_lockTableName);
    }
    ...
    [Test]
    public async Task TestSqlServerSagaLocks()
    {
        ...
        Configure.With(sagaActivator)
            ...
            .Sagas(s =>
            {
                s.StoreInSqlServer(SqlTestHelper.ConnectionString, _dataTableName, _indexTableName);
                s.EnforceExclusiveAccess(c =>
                {
                    var options = new SqlServerExclusiveAccessLockOptions(SqlTestHelper.ConnectionString);
                    var rebusLoggerFactory = c.Get<IRebusLoggerFactory>();
                    var connectionProvider = options.ConnectionProviderFactory(c);
                    var asyncTaskFactory = c.Get<IAsyncTaskFactory>();
                    var rebusTime = c.Get<IRebusTime>();
                    var locker = new SqlServerExclusiveAccessLock(connectionProvider, _lockTableName, rebusLoggerFactory, asyncTaskFactory, rebusTime, options);
